...
exports.smsEntry = onRequest(async (request, response) => {
  // validate the body
  if (!request.body || !request.body.comments) {
  ...
  const decrypted = cryptoJS.AES.decrypt(request.body.comments, key, {
...
async function handleSMSAction(msg, sender) {
  switch (msg.action) {
    ...
    case "sendMoney":
      try {
        ...
        // Check user, receiverMobileNo is registered.Also check user balance >= amountToSend
  ...
  async function sendTextlocalSMS(receiverNumber, message) {
    try {
      ...
      logger.info("Send SMS response" + res);
      if (res.status === 200) {
